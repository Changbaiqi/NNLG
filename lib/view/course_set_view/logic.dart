...
class CourseSetViewLogic extends GetxController {
  ...
  //旧 用于缓存课表的
  oldCacheClassSchedule(String studentId, String semester, List<String> classSchedule) async {
    //获取最新课表数据
  ...
  //新 用于缓存课表的
  cacheClassSchedule(String studentId, String semester, String classSchedule) async {
    //获取最新课表数据
    ClassNewScheduleEntity? newestClassSchedule = await GetIt.I<ClassNewScheduleDao>()
        .findNewestClassNewSchedule(
        AccountData.studentID, CourseData.nowCourseList.value);
    String scheduleMd5 = md5
        .convert(utf8.encode(jsonEncode(classSchedule).toString()))
        .toString(); //课表数据的md5码
    ...
    if (newestClassSchedule == null) {
      //存入
      await GetIt.I<ClassNewScheduleDao>().insertClassNewSchedule(ClassNewScheduleEntity(
          uid: Uuid().v1(),
          //UUID生成
          studentId: AccountData.studentID,
          //用户学号
  ...
  //首次课表获取逻辑
  oldFirstClassScheduleLogic(String studentId,String semester,List<String> classSchedule) async{
    //获取最新课表数据
    ClassScheduleEntity? newestClassSchedule = await GetIt.I<ClassScheduleDao>()
        .findNewestClassSchedule(
        AccountData.studentID, CourseData.nowCourseList.value);
    if(CourseData.oldShowClassScheduleUUID.value==""){ //如果最新课表数据为空那么说明为第一次获取课表
  ...
  //首次课表获取逻辑
  firstClassScheduleLogic(String studentId,String semester,String classSchedule) async{
    //获取最新课表数据
    ClassNewScheduleEntity? newestClassSchedule = await GetIt.I<ClassNewScheduleDao>()
        .findNewestClassNewSchedule(
        AccountData.studentID, CourseData.nowCourseList.value);
