...
class ClassScheduleWidget extends StatelessWidget {
  ...
  List<dynamic> columTimeList=[
    DateTime.now(),DateTime.now(),DateTime.now(),DateTime.now(),DateTime.now(),DateTime.now(),DateTime.now()
  ];
  static final _weekViewKey = GlobalKey();


  ClassScheduleWidget({this.tableJson,this.isNoon=false,columTimeList,rowTimeList}){
  ...
  Widget build(BuildContext context) {
    //用于构建标记格子数组
    for(int i=0;i<12;++i) isOccupy.add([{"state": false,"table": {}
        },{"state": false,"table": {}
        },{"state": false,"table": {}
        },{"state": false,"table": {}
        },{"state": false,"table": {}
      ...
      list.add(Positioned(
        child: InkWell(
          child: Container(
            ...
            width: Get.context!.width / 8 * (element["columEnd"]-element["columStart"]+1),
            child: Padding(
   ...
   * [return] 
   */
  Widget _backgroundLine({bool noonSwitch=false}){
    //根据tableJson导入标记格子
